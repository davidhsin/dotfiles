...
-- when you need it for debugging
...
-- 必須要設定好lsp clients？
require('symbols-outline').setup({ -- has installed, but doesn't work ;(
...
vim.api.nvim_set_keymap('n', '<C-g>', ":lua require('fzf-lua').grep()<CR>", { noremap = true, silent = true })
...
require'fzf-lua'.setup {
  ...
  winopts = {
    ...
    preview = {
      ...
      scrollbar      = 'border',        -- `false` or string:'float|border'     border is perfect in my opinion :)
      ...
      delay          = 10,--origin100             -- delay(ms) displaying the preview
